...
import com.tang4j.security.util.UrlMatcher;
...
public class MyInvocationSecurityMetadataSourceService implements FilterInvocationSecurityMetadataSource {
    ...
    public Collection<ConfigAttribute> getAttributes(Object object) throws IllegalArgumentException {
        ...
        if (resourceMap == null) {
            ...
            synchronized (MyInvocationSecurityMetadataSourceService.class) {
                if (resourceMap == null) {
                    loadResourceDefine();
                }
            }
        ...
        log.info(MyInvocationSecurityMetadataSourceService.class.getName(), "请求 url ：" + url);
    ...
    public void loadResourceDefine() {
        ...
        //获取所有分配的角色
        List<SysRole> roleList = MyUserDetailsService.ALL_ROLES;
        if (!CollectionUtils.isEmpty(roleList)) {
            for (SysRole role : roleList) {
                //授权标识
                String authorizedSigns = role.getCode().trim();
                ConfigAttribute configAttributes = new SecurityConfig(authorizedSigns);
                //获取角色对应菜单资源
                List<SysMenuResource> moduleList = MyUserDetailsService.COMMON_MENUS;
                assert moduleList != null;
                for (SysMenuResource module : moduleList) {
                    //请求url
                    String url = !StringUtils.isEmpty(module.getRequestUrl()) ? module.getRequestUrl().trim() : "";
        ...
        log.info(MyInvocationSecurityMetadataSourceService.class.getName(), resourceMap);
